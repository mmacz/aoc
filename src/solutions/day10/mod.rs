...
fn which_destroyed_at(input: &str, best_place: (i32, i32), destroy_at: i32) -> (i32, i32) {
    ...
    for &a in asteroids.iter() {
        ...
        by_angle.entry(angle).or_insert(Vec::new()).push((dist, a));
    }
...
mod tests {
    ...
    fn test_get_angle_above() {
        ...
        assert!(
            (angle - 0.).abs() < std::f64::EPSILON,
            "Expected: 0 vs Actual: {}",
            angle
        );
    }
    ...
    fn test_get_angle_right() {
        ...
        assert!(
            (angle - 90.0).abs() < std::f64::EPSILON,
            "Expected: 90 vs Actual: {}",
            angle
        );
    }
    ...
    fn test_get_angle_down() {
        ...
        assert!(
            (angle - 180.0).abs() < std::f64::EPSILON,
            "Expected: 180 vs Actual: {}",
            angle
        );
    }
    ...
    fn test_get_angle_left() {
        ...
        assert!(
            (angle - 270.0).abs() < std::f64::EPSILON,
            "Expected: 270 vs Actual: {}",
            angle
        );
    }
